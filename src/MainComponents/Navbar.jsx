...
import { AiOutlineClose, AiOutlineMenu , AiOutlineLeft } from 'react-icons/ai';
import { Link, useMatch, useResolvedPath } from "react-router-dom";
import Avatar from './Avatar';
import Points from './Points';

const Navbar = ({ userRole, children }) => {
  const [nav, setNav] = useState(window.innerWidth > 1024);
  ...
  return (
    <div className='relative h-screen'>
      {/* Navbar */}
      <div className='flex justify-between items-center h-24 px-4 text-secondary bg-primary drop-shadow-md z-50'>
      ...
      <div ref={navRef} className={`${
        nav
          ? 'left-0 top-0 xl:w-[20%] md:w-[30%] w-[90%] h-screen absolute bg-secondary ease-in-out duration-500 z-50 '
          : 'left-[-100%] top-0 md:w-[30%] w-[30%] h-screen absolute bg-secondary ease-in-out duration-1000 z-50'
        }`}>
        <div className='bg-primary h-24 items-center py-7 sm:px-2'>
          <div onClick={handleNav} className=' flex justify-center'>
             <AiOutlineLeft size={20} className='my-auto' /> 
            <Link to="/" className="">
      ...
      <div className={`max-h-screen  ${nav ? 'xl:ml-[20%] lg:ml-[30%]' : ''}`}>
        {children}
      </div>
    </div>
  );
